...
if ($handle = opendir('.')) {
	while (false !== ($entry = readdir($handle))) {
		if ($entry != "." && $entry != ".." && $entry != "php_errorlog" && $entry != "lang" && substr($entry,0,1) != "_" && strpos($entry, ".") === FALSE && substr($entry,-4) != ".log" && substr($entry,0,6) != "images" && substr($entry,0,8) != "trailers" && substr($entry,0,9) != "error_log") {						if ($entry == "paperlander") {				echo '<a href="sheet.php?p='.$entry . str_replace('?', '&', $languageQuery).'">PaperLander</a><br />';			} else {
...
if ($handle = opendir('.')) {
	while (false !== ($entry = readdir($handle))) {
		if ($entry != "." && $entry != ".." && $entry != "php_errorlog" && $entry != "lang" && substr($entry,0,1) != "_" && strpos($entry, ".") === FALSE && substr($entry,-4) != ".log" && substr($entry,0,6) != "images" && substr($entry,0,8) != "trailers" && substr($entry,0,9) != "error_log") {			if ($entry == "paperlander") {				echo '<li><a href="sheet.php?p='.$entry. str_replace('?', '&', $languageQuery).'">PaperLander</a></li>';			} else {				echo '<li><a href="sheet.php?p='.$entry. str_replace('?', '&', $languageQuery).'">'.ucwords(str_replace("_", " ", $entry)).'</a></li>';			}
		}
	}
}
closedir($handle);
